...
window.gameFunctions.gameOverride = function(){
	...
	var baseZoomFast = this.nt.zoomFast;
	this.nt.__defineSetter__("zoomFast", function(val){
		baseZoomFast = val;
	});
	this.nt.__defineGetter__("zoomFast", function(){
	...
	var inputKeyPressedBase = this.fe.Z;
	this.fe.Z = function(e){
		if(window.gameVars)
		{
	...
	var inputMousePressedBase = this.fe.te;
	...
	var inputMouseDownBase = this.fe.$;
	this.fe.$ = function(){
		if(window.gameVars && window.gameVars.Input.Cheat.RepeatFire)
			return false;
