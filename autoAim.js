window.autoAim = function(obfuscate, game, variables) {
	...
	var reloadingTexture = window.PIXI.Texture.fromImage("https://cdn.rawgit.com/PowerOfUniverse/Surviv.io-Cheats/master/ChromeExtension/images/reload.svg");
	var healingTexture = window.PIXI.Texture.fromImage("https://cdn.rawgit.com/PowerOfUniverse/Surviv.io-Cheats/master/ChromeExtension/images/heal.svg");
	...
	var detectEnemies = function() {
		...
		var allPlayers = game.scope[obfuscate.playerBarn.main][obfuscate.playerBarn.players];
		var playerArray = [];
		...
		var allPlayersStr = "";
		var teamsTab = document.getElementById("ui-cheat-tab-2");
		if (teamsTab) {
			teamsTab.innerHTML = "";
			...
			for (let team in allPlayerDict) {
				...
				teamsTab.innerHTML += `<p><strong>TEAM ${team}:</strong> ${allPlayersStr}</p>`;
			}
		}
	...
	var showTargetEnemyNick = function() {
		...
		state.player.nameText.style.fontSize = 50;
		state.player.nameText.style.fill = "#0077ff";
	}
